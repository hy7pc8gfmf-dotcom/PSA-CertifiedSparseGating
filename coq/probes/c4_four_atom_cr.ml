...
let mu_c4 =
  { qnum = (Zpos (XI (XO (XO (XI (XI (XO (XO (XO (XO (XO (XI (XI (XO
    XH)))))))))))))); qden =
    (Coq_Pos.of_succ_nat
      (of_num_uint (UIntDecimal (D3 (D3 (D9 (D1 (D9 Nil)))))))) }

(** val c4_ladder_zero : nat -> cRComplex **)

let c4_ladder_zero _ =
  cRzero cRealConstructive

(** val c4_mu_cauchy : q -> cRcarrier **)

let c4_mu_cauchy _ =
  cRealConstructive.cR_of_Q mu_c4

(** val c4_combo_cauchy : (nat -> cRcarrier) -> cRComplex **)

let c4_combo_cauchy c =
  cRcombo cRealConstructive (S (S (S O))) c c4_ladder_zero

(** val c4_norm_sq_cauchy : (nat -> cRcarrier) -> cRcarrier **)

let c4_norm_sq_cauchy c =
  cRnorm_sq cRealConstructive (c4_combo_cauchy c)

(** val c4_mu4_window : (z, positive) prod **)

let c4_mu4_window =
  Pair ((qmult mu_c4 { qnum = (Zpos (XO (XO XH))); qden = XH }).qnum,
    (qmult mu_c4 { qnum = (Zpos (XO (XO XH))); qden = XH }).qden)
