...
(** val cr_win_sum :
    constructiveReals -> (nat -> cRcarrier) -> nat -> cRcarrier **)

let rec cr_win_sum r f = function
| O -> f O
| S i -> r.cRplus (cr_win_sum r f i) (f (S i))

(** val cr_debt :
    constructiveReals -> (nat -> nat -> cRComplex) -> nat -> nat -> cRcarrier **)

let cr_debt r u n t =
  r.cRplus (r.cR_of_Q { qnum = (Zpos XH); qden = XH })
    (r.cRopp (cr_win_sum r (fun k -> cRnorm_sq r (u n k)) t))

(** val cr_list_sum :
    constructiveReals -> (nat -> cRcarrier) -> nat list -> cRcarrier **)

let rec cr_list_sum r g = function
| Nil -> r.cR_of_Q { qnum = Z0; qden = XH }
| Cons (x, xs) -> r.cRplus (g x) (cr_list_sum r g xs)

(** val cr_kept : (nat -> nat) -> nat -> nat list -> nat list **)

let cr_kept v t i =
  filter (fun i0 -> Nat.leb (v i0) t) i

(** val cr_pruned : (nat -> nat) -> nat -> nat list -> nat list **)

let cr_pruned v t i =
  filter (fun i0 -> negb (Nat.leb (v i0) t)) i

(** val tau_cr_ladder_zero : nat -> nat -> cRComplex **)

let tau_cr_ladder_zero _ _ =
  cRzero cRealConstructive

(** val cr_win_sum_cauchy : (nat -> cRcarrier) -> nat -> cRcarrier **)

let cr_win_sum_cauchy f t =
  cr_win_sum cRealConstructive f t

(** val cr_debt_cauchy : nat -> nat -> cRcarrier **)

let cr_debt_cauchy n t =
  cr_debt cRealConstructive tau_cr_ladder_zero n t

(** val cr_list_sum_cauchy : (nat -> cRcarrier) -> nat list -> cRcarrier **)

let cr_list_sum_cauchy g l =
  cr_list_sum cRealConstructive g l
