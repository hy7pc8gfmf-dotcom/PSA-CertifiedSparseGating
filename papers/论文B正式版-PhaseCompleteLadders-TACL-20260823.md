...
> 实验数据不变；配套论文 A（Coq 形式化）代码状态按当前基态（合并版 `ca_merged_full_84_m2.v`（102174 行，84 模块分区，绿产物追加路线：E153 绿基线 87632 + M2/G-7/G-9 批次 + G-10/G-11/G-13/CS-21/CS-23/CS-22/Z2b 批次（2026-09-01 追加批次）+ z3b 批次 + C 系列/G-12 批次（2026-09-02 追加批次，云端 CI 全局绿：Rocq 9.0 + mathcomp 2.6 真环境 + coqchk）），
...
- **代码状态**：论文 A 的 z 区 20 探针（grid_ortho + parseval/partial/pairbound/rowsum/
  pairdirichlet/incoherence/row_rip/c4_instance/welch/uncertainty_cr/g8_synthesis_cr/g1_norm_closed/g2_mu_adj + 构造性族 pi_cr_m1a/m1b/sin_cr_m2/sqrt_cr_m3/s7_s9_mono/decidability_premium_cr）已 pro 化并入合并版 `ca_merged_full_84_m2.v`（102174 行，84 模块分区，云端 CI 全局绿）——本证书链依赖的 parseval/partial/pairbound/rowsum/
