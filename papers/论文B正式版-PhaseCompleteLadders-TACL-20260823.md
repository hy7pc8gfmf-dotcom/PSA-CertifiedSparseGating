...
> 正式版（对齐当前代码基态）。来源：论文 B 草稿清洗（删除会话/评审/版本内部注记），
> 实验数据不变；配套论文 A（Coq 形式化）代码状态按 当前基态（合并版 `ca_merged_full_25.v`，87632 行 / 57 顶层 Module = 55 模块分区，
