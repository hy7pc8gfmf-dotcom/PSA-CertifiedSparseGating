...
- **CS 战役探针（2026-08-29）**：`probe_taugrid.v`（17 Qed，τ 裁剪最优性）/ `probe_taugrid_cr.v`（13 Qed 零公理，构造性孪生，提取 `z/extract/taugrid_cr.ml` 已 OCaml 编译验证）/ `probe_cs.v`（29 Qed，CS 生产线）/ `probe_cs5.v`（5 Qed，offset 无关性）——四模块合计 64 Qed，独立编译 EXIT=0，归档 30模块 SHA 一致，**已并入合并版 25（2026-08-30 M2 批次再生，91606 行 / 67 模块分区，本地 2.5 编译 EXIT=0）**。
