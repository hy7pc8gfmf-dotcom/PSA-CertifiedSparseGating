...
**反射检查器 vs 通用决策过程**：与 CAD 类决策过程工作相比，`frame_check_instance` 不是通用决策过程，而是针对 Gershgorin 行和条件的专用**充分条件**判定——保守健全（假阴性 49.1% 已量化、精确层充要性由 G-3 证明、假阴性归因于松弛层而非检查器逻辑），以专用性换取可判定性与运行时轻量（典型阶梯 < 1ms 量级）。
