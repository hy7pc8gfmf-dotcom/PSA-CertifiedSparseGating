...
- 贡献清单（与代码一一对应）：
  ...
  8. **反射检查器及其健全性**（FrameCheckInstance，已完成）：`frame_check_instance`
     把"任意梯子 → μ≤4/5"做成可判定有理判定（floor-sqrt 有理化 + nat 分数行和），
...
- 梯子生成器 next = max(C·last+1, last+2)（非精确几何——证书必须认证实际值，
  这是需要运行时检查器而非纸面公式的理由）；
...
frame_check_instance_sound (I) : 0 < length I → frame_check_instance I = true →
  ...
  (* 反射检查器健全性：判定通过 ⟹ 任意梯子 μ=4/5 框架界，M4 收尾③ *)
...
- 对象：C=4 可见子梯 [3,13,53,213]（N=512 窗口内列范数=1；更长频带在窗口内
  近似线性斜坡、范数→0——全梯子认证得平凡界，窗口自适应认证是正确设计）。
...
- **从实例到通用判定（会话 9-11）**：同一配方推广为反射检查器 `frame_check_instance`
  （任意梯子 → μ≤4/5 布尔判定，floor-sqrt 有理化 + nat 分数行和，全程可判定有理算术），
  其健全性主定理 `frame_check_instance_sound` 已 Qed——**实例证书不再逐梯子手工验算，
...
- **检查器的保守性（充分非必要）**：`frame_check_instance = true` 是框架界的
  **充分非必要条件**——floor-sqrt 有理松弛的保守因子约 1.5×，存在满足
  Gershgorin 条件但因保守上界超 4/5 而被误拒的合法梯子（假阴性）。典型案例：
  论文 B 的七带基线梯子 E5'' [3,7,15,31,63,127,255] 被检查器判 false，**不代表它不满足框架条件**，仅代表
  有理松弛不足以证明它。正是这一不完备性使**复合证书成为必要**（而非补丁式
  workaround）：检查器拒绝整体 ⟹ 核-边缘分解 ⟹ 认证核 + 能量预算
  （§5.3 端到端冠军证书，✅ 已完成）。宁可误拒、绝不放行的保守取向是可判定性的
  设计选择，须与"部分证书"机制配套使用。
  **评审 10 定位修正（2.4）**：系统扫描（114 梯子×5 族，`frame_check_scan.py`，
  会话 18）实测假阴性 **56/114（49.1%，占拒绝 70.9%）**，且**集中于"有用"梯子**
...
- **普适性论证**：溢价机制与具体梯子无关——任何"超越界 → 有理界"的证书化
   ...
   **反向警示（形式化审查 §II.B 强化，定稿必读）**：检查器返回 `false` **不是**
   ...
   梯子仍可由复合证书覆盖（§5.3 七带即此情形：其精确相干行和仅 0.135 ≤ 4/5，
...
3-seed 实验中，E5'' 七带 [3,7,15,31,63,127,255] 与 C=4 并列冠军
（8× 均值 12.40±0.74 vs 12.75±0.34，差距 -2.8% 在噪声带内）；C=2 第三
（13.84），C=3 系统性最差（22.86，归因为间距非噪声）。冠军身份对论文 A 免疫：
- **直接证书**（C=4 胜）：certified_c4_frame_bounds 直接覆盖；
- **T8 复合证书**（E5'' 胜，**已并入，会话 9**）：其隔带子核 [3,15,63,255]
  T8CoreCertificate 模块 11 Qed 零 classic，μ=4/5 覆盖冠军的认证核。
两种排位论文均有主定理覆盖。
- **端到端冠军证书（✅ 已完成，会话 14 Qed）**：顶层组合定理
  ...
  边带能量"叙述是装配前的设计稿，最终实现收敛为单一定理下的对称界）。
...
- **反射检查器不完备性：影响与缓解（评审 A3，会话 18 系统扫描补强）**：`frame_check_instance` 是**充分非
  必要**——实际可认证梯子被判 false（假阴性）。**系统扫描统计（114 梯子 × 5 族：
  ...
  E1 随机族 3/18（该族多为真负：稠密梯子精确行和确 > 4/5）。代表性复核：C4 与
  T8 核 true、E5'' 七带与 C=2 八带 false。
  **细化发现**：E5'' 七带在窗口 255 的**精确**相干行和仅 0.135 ≤ 4/5——其实质
  **可认证**；检查器拒绝来自与窗口无关的保守对界（√(n₁n₂)/(2(n₂−n₁)) 远大于
  精确相干 |sin(Tθ/2)/(T·sin(θ/2))|）。故复合证书不是"挽救不可认证梯子"，而是
  对**保守预筛器误拒梯子**给出的紧证书（其 `coh_e5` 即 21 对 δ 表的精确相干界）。
  **缓解（已实现）**：(a) **部分证书组合**——被拒梯子的核-边带分解（T8 核通过
...
- **相干熵桥接（评审 5 增值方向 1，核心桥梁定理已 Qed，会话 18）**：外推崩坏的主因候选为
  ...
  **‖softmax z − softmax z'‖₁ ≤ e^{2·coh·delta} − 1**——相干上界 ⟹ softmax 输出
  ...
  （具体梯子 + 学习投影层）为后续工作。当前为**已证抽象桥梁 + 未实例化**状态；
...
- **联合主张边界（元定理，两篇论文实际共同证明的部分）**：对通过检查器的梯子
  （如 C=4）：机器可检查的框架界 μ≤4/5，任意系数下表示稳定性；对未通过检查器
  的冠军七带：隔带子核 μ≤4/5 + 边带能量有界（T8CoreCertificate 11 Qed +
  `row_dropped_energy_bound`）。**从"能量控制"到"外推 ppl 提升"的桥梁未经
  形式化**，依赖论文 B 的经验观察——两文联合可证部分止于此。相应地，一个
  如实呈现的张力：认证梯子（C=4，8× 均值 12.75）与未认证冠军（12.40）呈
  **温和负相关**——表达性与可证明性此消彼长，复合证书覆盖冠军的可证骨架
  而非其外推增益（见论文 B §7）。
...
- **常数代价（如实报）**：与 2D（K0 = Rmax 8C³/4，但衰减引理要求**两轴索引都不同**）相比，
  ...
  形式化量化**：各向同性（等轴）梯子是 3D 相干最坏情形，非等轴设计可望收回 ×2；
  1D/2D 现有证书（μ=4/5 等）不受此修正影响；
- **数值紧度裁决（诚实，双因子分解）**：三轴实例 [3,13,53]（C=4，3³=27 基函数）——
  ...
  **编码因子 K0′ = C³/2 = 32**（混合进制扁平索引下等轴退化配置的病理覆盖——
  改变扁平化编码（格雷码类方案）或采用非等轴梯子可望收回此因子）与
...
- **双轨设计（窄轨/宽轨并列声明）**：/2 修正是"宽陈述"的价格而非高维强加给
  ...
  存在性叙事**）。仅需 1D/2D 的应用场景应采用窄轨；宽轨家族 2D/3D/4D 成员齐备
...
- **严格增长延拓 `seq_ext`**（E075 关联）：窗口内取 `nth i vals`；窗口外按几何闭式
  ...
  `seq_ext_growth_R` 等族）——把有限梯子提升为全索引序列以满足 `abstract_unconditional_basis`
  骨架的全局增长前提；
...
- **`frame_check_instance`（会话 9 落地，10+11 完成健全性）**：任意梯子 →
  反射布尔判定 μ≤4/5。PSA_framework.v 定义 + `frame` CLI + FFI wrapper。
  原生 int 实现（main.ml 的 frame_check_instance_int，镜像 Coq 定义）规避 Peano
  nat 大数乘法的字节码栈溢出（提取的 Peano 版对 ≥7 带会炸）；key 梯子回归正确
  （C4/T8 true、E5''/C2b false；psa_guard.exe 实测 C=6 大梯子值 ~1339 不溢出）。
  **范围前提（如实声明）**：int 镜像与 Coq `nat` 的等价仅对**中间量不超过机器
  字长（OCaml int，63-bit）**的梯子成立；实验中梯子值 ≤255 安全，对极大输入
  （≥10⁶ 级）`sparse` 等走 Peano 提取的子命令仍会栈溢出（已实测）——故运行时
  健全性保证的量化范围是**小整数梯子**，并非 ∀ 梯子；彻底消除可改用 `Z`
  ...
  **定量安全阈值（会话 18 扫描修正，原"n_max<2^20、m<100 无溢出"不成立）**：
  ...
  （指数增长），与单个 pair_den 上界无关。系统扫描（114 梯子，`frame_check_scan.py`）
  实测：末带 < 2^20 的梯子中仍有 14/89（15.7%）exe 与 Coq 语义分歧——如 C=6-sparse
  ...
  阈值须逐梯核算 ∏ pair_den < 2^63**（或直接改用任意精度）。**彻底消除：`Zarith`（或
...
- **反射层纠错实例（形式化发现并修复真 bug，可写进 paper）**：反射层原
  ...
  不一致（[3,13] 误判 false、C4 行和退化为 (0,0) 真空通过）；加 `orig` 参数
...
## 7. 公理记账（诚实性小节，对齐 2026-08-20 基态：165 项全零 classic）
...
## 10. Limitations
- **高维证书的诚实定位**：3D 定理 `tensor_product_unconditional_basis_3d` 是形式化
...
- **形式化的适用域以结构存在为前提**：框架界与衰减界的全部证明依赖频率梯子
  （seq 存在性）；对无位置结构的编码（NoPE 类）本框架**无任何可证陈述**。
  论文 B 实证给出这笔"定量交易曲线"：有结构端以盘内 ppl ~2× 劣化
  （4.46 vs 9.06）为代价承受 OOD 相位混淆风险（外推退化 2.7–2.9× vs NoPE 的
  1.27×）；形式化锁定的正是交易中**有结构的一端**（可证性 + 盘内最优性），
  无结构端的平缓外推在本框架的可证范围之外（两文联合边界，见论文 B §8：
  无证书的测试时修复 rope-NTK 与带证书的免手术方案在同一基准上构成
  不可比的实证-形式化张力）。
...
诚实地讲，3D/4D 证书当前是非紧的存在性结果，定位为 **"Theoretically Composable,
Practically Non-Tight"**，其价值在架构可组合性与维度推广叙事，而非常数本身；
1D 证书（μ=4/5）保持紧且不受影响。

---
*配套：论文 B（实证+相位律）、诚实性支柱实验设计、定理栈增强路线。*
